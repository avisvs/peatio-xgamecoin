RSpec.describe Peatio::Xgamecoin::Blockchain do
  context :features do
    it 'defaults' do
      blockchain1 = Peatio::Xgamecoin::Blockchain.new
      expect(blockchain1.features).to eq Peatio::Xgamecoin::Blockchain::DEFAULT_FEATURES
    end

    it 'override defaults' do
      blockchain2 = Peatio::Xgamecoin::Blockchain.new(cash_addr_format: false)
      expect(blockchain2.features[:cash_addr_format]).to be_falsey
    end

    it 'custom feautures' do
      blockchain3 = Peatio::Xgamecoin::Blockchain.new(custom_feature: :custom)
      expect(blockchain3.features.keys).to contain_exactly(*Peatio::Xgamecoin::Blockchain::SUPPORTED_FEATURES)
    end
  end

  context :configure do
    let(:blockchain) { Peatio::Xgamecoin::Blockchain.new }
    ...
    it 'currencies and server configuration' do
      currencies = [{ id: :mrja,
                      ...
                      options: {} }]
      settings = { server: 'http://user:password@127.0.0.1:55466',
  ...
  context :latest_block_number do
    ...
    let(:server) { 'http://user:password@127.0.0.1:55466' }
    let(:server_without_authority) { 'http://127.0.0.1:55466' }
    ...
    let(:blockchain) do
      Peatio::Xgamecoin::Blockchain.new.tap {|b| b.configure(server: server)}
    end
    ...
    it 'raises error if there is error in response body' do
      stub_request(:post, 'http://127.0.0.1:55466')
        .with(body: { jsonrpc: '1.0',
                      method: :getblockcount,
                      params:  [] }.to_json)
  ...
  context :build_transaction do
    ...
    context 'two vout tx' do
      ...
      let(:blockchain) do
        Peatio::Xgamecoin::Blockchain.new.tap { |b| b.configure(currencies: [currency]) }
      end
    ...
    context 'multiple currencies' do
      ...
      let(:blockchain) do
        Peatio::Xgamecoin::Blockchain.new.tap do |b|
  ...
  context :fetch_block! do
    ...
    let(:server_without_authority) { 'http://127.0.0.1:55466' }
    let(:blockchain) do
      Peatio::Xgamecoin::Blockchain.new.tap { |b| b.configure(server: server, currencies: [currency]) }
    end
