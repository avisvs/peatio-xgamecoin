RSpec.describe Peatio::Xgamecoin::Client do
  let(:uri) { "http://user:password@127.0.0.1:55466" }
  let(:uri_without_authority) { "http://127.0.0.1:55466" }

  before(:all) { WebMock.disable_net_connect! }
  after(:all) { WebMock.allow_net_connect! }

  subject { Peatio::Xgamecoin::Client.new(uri) }
    ...
    context :methodnotfound do
      ...
      it do
        expect{ subject.json_rpc(:methodnotfound) }.to \
          raise_error(Peatio::Xgamecoin::Client::ResponseError, "Method not found (-32601)")
      end
    end
    ...
    context :notfound do
      ...
      it do
        expect{ subject.json_rpc(:notfound) }.to \
          raise_error(Peatio::Xgamecoin::Client::Error)
      end
    end
    ...
    context :connectionerror do
      ...
      it do
        expect{ subject.json_rpc(:connectionerror) }.to \
          raise_error(Peatio::Xgamecoin::Client::ConnectionError)
      end
    end
  end
end
